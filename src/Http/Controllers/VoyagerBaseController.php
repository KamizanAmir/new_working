...
class VoyagerBaseController extends Controller
{
    ...
    // {
    //     $groupBy = $request->get('group_by', 'monthly'); // Default to monthly if not specified

    //     $query = TrainingPlan::select(
    //         DB::raw('count(*) as count'),
    //         DB::raw($this->getGroupBySql($groupBy))
    //     )
    //     ->whereYear('created_at', '=', date('Y')) // Limit to the current year, you can adjust this
    //     ->groupBy('time_group')
    ...
    protected function getGroupBySql($groupBy)
    {
        switch ($groupBy) {
            case 'weekly':
                return "DATE_FORMAT(created_at, '%x-%v') as time_group"; // ISO week and year
            case 'monthly':
                return "DATE_FORMAT(created_at, '%Y-%m') as time_group"; // Month and Year
            case 'yearly':
                return "YEAR(created_at) as time_group"; // Year
            default:
                return "DATE_FORMAT(created_at, '%Y-%m') as time_group";
        }
    }
